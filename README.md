...
- 在应用内 API 设置里粘贴 Key：会固定保存在这台电脑里，不会显示明文。
- 开发模式下创建 `.env.local`：适合开发者本地调试。

项目记忆、最近会话、聊天记录和应用内 API Key 都会保存在当前电脑本地。退出应用、重新打开或安装新版后会继续保留。桌面版使用固定本地地址保存数据，并且只允许同时打开一个 Jindou Agent，避免出现两个应用窗口读写两份记录。
